...
def _get_stripe_config():
    """Get Stripe configuration from Flask app config."""
    return current_app.config

# Set up Stripe API key - this will be called when the service is used
def _init_stripe():
...
def create_checkout_session(user_id, success_url, cancel_url):
    ...
    try:
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            line_items=[
                {
                    "price": config['STRIPE_PRICE_ID_MONTHLY'],  # Updated to use the correct key
                    "quantity": 1,
                }
            ]
        )
